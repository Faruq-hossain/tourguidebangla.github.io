...
if (isset($_POST['form1'])) {
    ...
    if ($valid == 1) {
        ...
        $q = $pdo->prepare("SHOW TABLE STATUS LIKE 'slider_tour'");
        $q->execute();
        $result = $q->fetchAll();
        ...
        $final_name = 'slidertour_' . $ai_id . '.' . $ext;
        move_uploaded_file($path_tmp, '../touruploads/' . $final_name);

        $q = $pdo->prepare("INSERT INTO slider_tour (slider_title,slider_subtitle,slider_button_text,slider_button_url,slider_photo) VALUES (?,?,?,?,?)");
        $q->execute([$slider_title, $slider_subtitle, $slider_button_text, $slider_button_url, $final_name]);

        $success_message = 'Slider Tour is added successfully!';
    }
}
?>
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Add Tour Slider</h1>
    </div>
</div>
