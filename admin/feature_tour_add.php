...
if(isset($_POST['form1'])) {
    ...
    $q = $pdo->prepare("SHOW TABLE STATUS LIKE 'feature_tour'");
    $q->execute();
    $result = $q->fetchAll();
    ...
    $q = $pdo->prepare("INSERT INTO feature_tour (feature_icon) VALUES (?)");
    $q->execute([$_POST['feature_icon']]);

    for($i=0;$i<count($arr1);$i++)
    {
        $q = $pdo->prepare("INSERT INTO feature_tour_language (feature_id,lang_code,feature_title,feature_text) VALUES (?,?,?,?)");
        $q->execute([$ai_id,$arr1[$i],$arr2[$i],$arr3[$i]]);
    }

    $success_message = 'Feature Tour is added successfully!';
