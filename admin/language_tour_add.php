...
if(isset($_POST['form1'])) {
    ...
    $q = $pdo->prepare("INSERT INTO language_tour (lang_code,lang_name,lang_default) VALUES (?,?,?)");
    $q->execute([$_POST['lang_code'],$_POST['lang_name'],'']);

    $q = $pdo->prepare("SELECT * FROM feature_tour_language WHERE lang_code=?");
    $q->execute(['en']);
    $res = $q->fetchAll();
    foreach ($res as $row) {
        $arr1[] = $row['feature_id'];
    ...
    {
        $q = $pdo->prepare("INSERT INTO feature_tour_language (
                    feature_id,
                    lang_code,
                    feature_title,
                    feature_text
    ...
    $success_message = 'Language Tour is added successfully!';
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Add Tour Language</h1>
    </div>
</div>
