...
if (isset($_POST['form1'])) {
    ...
    if ($path == '') {
        ...
        if ($mime != 'image/jpeg' && $mime != 'image/png' && $mime != 'image/gif') {
            $valid = 0;
            $error_message .= 'Only jpg, png and gif files are allowed for post Tour photo<br>';
        }
    }
    ...
    if ($valid == 1) {
        ...
        $q = $pdo->prepare("SHOW TABLE STATUS LIKE 'post_tour'");
        $q->execute();
        $result = $q->fetchAll();
        ...
        $q = $pdo->prepare("INSERT INTO post_tour (
                        post_title,
