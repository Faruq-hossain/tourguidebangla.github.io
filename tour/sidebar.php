<div class="widget clearfix">
  ...
  <ul class="list-unstyled">
    ...
    <li>
      <article>
        <div class="news-thumb"> <a href="blog-detail.php?id=<?php echo $row['post_id']; ?>"><img src="../touruploads/<?php echo $row['post_photo']; ?>" alt="Popular news" style="width:100px;"></a> </div>
        <div class="news-content clearfix">
          <h4><a href="blog-detail.php?id=<?php echo $row['post_id']; ?>"><?php echo $row['post_title']; ?></a></h4>
          <span><?php echo month_number_to_detail($row['post_month']); ?> <?php echo $row['post_day']; ?>, <?php echo $row['post_year']; ?></a> </div>
      </article>
    </li>
      <?php
    }
...
<div class="widget">
  ...
  <ul class="list-unstyled arrow">
    ...
    <?php
    $q = $pdo->prepare("SELECT MIN(post_year) AS min_year FROM post_tour");
    $q->execute();
    $res = $q->fetchAll();
    foreach ($res as $row) {
    ...
    $q = $pdo->prepare("SELECT MAX(post_year) AS max_year FROM post_tour");
    $q->execute();
    $res = $q->fetchAll();
    foreach ($res as $row) {
    ...
    for($i=$max_year;$i>=$min_year;$i--)
    {
      ...
      for($j=12;$j>=1;$j--)
      {
        ...
        $q = $pdo->prepare("SELECT * FROM post_tour WHERE post_year=? AND post_month=?");
        $q->execute([$i,$j]);
        $res = $q->fetchAll();
        $tot = $q->rowCount();
        if($tot!=0)
